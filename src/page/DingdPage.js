...
//  新闻组件
export default class DingdPage extends Component {
    ...
    //         headerTintColor: '#fff',
    //         headerTitle: (
    //             <Text style={{ flex: 1, textAlign: 'center', fontSize: px2dp(16), color: '#fff' }}>我的客户</Text>
    //         ),
    //         headerRight: (
    ...
    //                     let url_ = "app/jsp/shanc/khedit.jsp?appu_id=" + username + "&appu_name=" + realname;
    ...
    fetchNewsData(start, end) {
        ...
        const res = selectDingdanList(user_.username, this.state.text, start, end); // api接口
        res.then((newsArr) => {
    ...
    sckehu(id) {
        ...
        Alert.alert('订单删除之后无法找回，请谨慎操作，确定删除吗？', '', [
            {
                text: '取消', onPress: () => {
    ...
    renderItem(rowData, index) {
        ...
        return (
            <View style={styles.wutuv}>
                ...
                {
                    rowData.state == '2' ?
                    <Image style={{ height: px2dp(60), width: px2dp(60), position: 'absolute', right: px2dp(15), bottom: px2dp(60), zIndex: 99, }} source={require('../img/icon1.png')} ></Image>
                    :null
                } 
              
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8, }}>
                    <Text style={styles.wutuvttitle} >{rowData.kc_name}  </Text>
                    <Text style={{ color: '#fb5903', fontSize: px2dp(18) }} ><Ionicons style={{ fontSize: px2dp(12) }} name='logo-yen' color="#888" />  {rowData.kc_ddje}  </Text>
                </View>

                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                    <View style={{ flex: 1 }}>
                        <Text style={styles.wutuvcontent} ><Icons style={styles.icon14} name='home' color="#888" />  企业：{rowData.kh_qymc}</Text>
                        <Text style={styles.wutuvcontent} ><Icons style={styles.icon14} name='user' color="#888" />  姓名：{rowData.kh_name}</Text>
                        <TouchableOpacity onPress={() => this.linking(rowData.kh_phone)}>
                ...
                {/* <Text style={styles.wutuvcontent} ><Icons style={styles.icon14} name='user-following' color="#888" />  客户职位：{rowData.zhiwu}</Text>
