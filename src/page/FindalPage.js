...
import {
    ...
    FlatList,
    Alert, TextInput,
    TouchableHighlight, ImageBackground, ScrollView,
    TouchableWithoutFeedback,
    TouchableOpacity,
    Dimensions,
    Image,
    ToastAndroid, ActivityIndicator, StatusBar
} from 'react-native';
...
import { getAlBy, getHangy, getBanners } from '../api/news';
...
export default class FindalPage extends Component {
    ...
    static navigationOptions = {
        ...
        tabBarIcon: ({ focused }) => {
            if (focused) {
                return (
                    <Image style={mstyles.img19} source={require('../img/al_sel.png')} />
                );
            }
            return (
                <Image style={mstyles.img19} source={require('../img/al_nor.png')} />
            );
        },
    };
    ...
    getHangy() {
        ...
        const res = getHangy(); // api接口
        res.then((newsArr) => {
            if(!isSpace(newsArr)&&newsArr.length > 0){
                this.setState({ // 设置状态
                    hangyDate: newsArr,
                });
            }
    ...
    getBanners() {
        // 取banner的时候 你需要考虑条件
        // const res = getAlBy(keyword, hangy, '1', 0, 100); // api接口
        const res = getAlBy('', '', '1', 0, 100); // api接口
        res.then((newsArr) => {
            this.setState({ // 设置状态
                bannerData: newsArr,
            });
    ...
    fetchNewsData(flag, start, end) {
        ...
        const res = getAlBy(keyword, hangy, '2', start, end); // api接口
        res.then((newsArr) => {
    ...
    loadMore() {
        ...
        this.fetchNewsData('loadMore', start, 10);
    ...
    render() {
        return (
            <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
                ...
                <View style={[mstyles.viewrowb, mstyles.padding8zy, mstyles.backhongs]}>
                ...
                {/* 1.设置背景 */}
                <ImageBackground source={require('../img/hongsebj.png')} style={{ height: px2dp(45), marginBottom: px2dp(10) }}>

                    <ScrollView
                        contentContainerStyle={{ alignItems: 'center', justifyContent: 'flex-end', }}
                        style={{ height: px2dp(45), borderBottomColor: '#e1e1e1', borderBottomWidth: px2dp(1) }}
                        horizontal={true} // 横向
                        showsHorizontalScrollIndicator={false}  // 此属性为true的时候，显示一个水平方向的滚动条。
                    >
                        <TouchableOpacity onPress={this.getBytype.bind(this, '')} >
                            <View style={{ justifyContent: 'flex-end', alignItems: 'center' }}>
                                <Text style={[{ fontSize: px2dp(14), paddingHorizontal: px2dp(15), color: "#fff", fontWeight: "bold", }]} >全部</Text>
                                {
                                    hangy == '' ? <Image style={{ height: px2dp(3), width: px2dp(16), marginTop: px2dp(4), borderRadius: px2dp(3) }} source={require('../img/js/xhxbs.png')} ></Image> :
                                        <Text style={{ height: px2dp(3), marginTop: px2dp(4), }}   > </Text>
                                }
                            </View>
                        </TouchableOpacity>

                        {
                            this.state.hangyDate.map((item, index) => {
                                return (
                                    <TouchableOpacity key={item.remark6} onPress={this.getBytype.bind(this, item.remark6)} >
                                        <View style={{ justifyContent: 'flex-end', alignItems: 'center' }}>
                                            <Text style={[{ fontSize: px2dp(14), paddingHorizontal: px2dp(15), color: "#fff", fontWeight: "bold", }]} >{item.remark6}</Text>
                                            {
                                                item.remark6 == hangy ? <Image style={{ height: px2dp(3), width: px2dp(16), marginTop: px2dp(4), borderRadius: px2dp(3) }} source={require('../img/js/xhxbs.png')} ></Image> :
                                                    <Text style={{ height: px2dp(3), marginTop: px2dp(4), }}   > </Text>
                                            }
